...
class StandardFullInferer(object):
    ...
    def evaluate_case(self, sess, model, batch_size, vs, vsegs):
        ...
        print(v_out_shape, y_out_list.shape, np.concatenate([v.seg_arr.reshape([1]+list(v.seg_arr.shape)) for v in vsegs]).shape)
        yr = y_out_list #[:,22:64,22:64,22:64].reshape(v_out_shape) #np.concatenate(y_out_list) #.reshape(v_out_shape)
        yr_labels = np.concatenate([v.seg_arr.reshape([1]+list(v.seg_arr.shape)) for v in vsegs])
