...
from oxnnet.record_writer import RecordWriter, StandardProcessTup, RecordReader
...
data_loader = StandardDataLoader(stride, segment_size_in, crop_by=crop_by, aug_pos_samps=False, equal_class_size=True)
    ...
    def __init__(self, batch_size, reuse=False, tf_record_dir=None, num_epochs=0, weighting=[1]*2):
        ...
        with tf.variable_scope("inference") as scope:
            ...
            with tf.variable_scope("pred"):
                ...
                #self.pred = tf.cast(locations[:,1],tf.float32)
            ...
            with tf.variable_scope("loss") as scope:
                ...
                self.loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=Y), name='cross_entropy')
                #self.loss_op = tf.reduce_mean(tf.losses.sigmoid_cross_entropy(Y, logits), name='cross_entropy')
