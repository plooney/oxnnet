"""Module containing classes to define a volume segment and
create volume segments from images and visa versa"""
import numpy as np
...
class VolumeSegment(object):
    ...
    def compute_indices(self, size_voxels, np_array_shape):
        ...
        self.inds = np.array([np.linspace(start, stop, stop-start + 1, dtype=np.int)
                              for start, stop in zip(ind_min_abs, ind_max_abs)])
        self.inds_rel = np.array([np.linspace(start, stop, stop-start+1, dtype=np.int)
                                  for start, stop in zip(ind_min_rel, ind_max_rel)])
...
class ImageHandler(object):
    ...
    def create_image_from_windows(self, vol_segs, image_shape, op_on_overlap=max_op_on_overlap):
        ...
        for vol in vol_segs:
            ...
            arr_recons[indices] = op_on_overlap(
                arr_recons, vol.seg_arr,
                indices, indices_rel) if op_on_overlap else vol.seg_arr[indices_rel]
        return arr_recons
