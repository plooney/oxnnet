...
class ImageHandler(object):
    def image_to_vols(self, image_arr,stride,window_shape, mask_arr=None,crop_by=0, add_rnd_offset=False):
        ...
        for index in np.ndindex(*divisions):
            ...
            if add_rnd_offset:
                ...
                rnd_offset = np.array([np.random.randint(w1,w2+1)
                                       for w1,w2 in zip(-offset_mag,offset_mag)])
                start_voxel += rnd_offset
            add_vol = True
