        ...
        with tf.get_default_graph().as_default():
            ...
            with tf.Session() as sess:
                ...
                for tup in test_data:
                    ...
                    dices[tup[0]] = dice
                    print("Median: {}, Max: {}, Min: {}".format(np.median(list(dices.values())), np.max(list(dices.values())), np.min(list(dices.values()))))
                print(dices)

    def feats(self, save_dir, test_data, model_file, module, batch_size):
        with tf.get_default_graph().as_default():
            #config = tf.ConfigProto()
            #config.gpu_options.allow_growth=True
            avg_time = 0
